@php
    ...
    $logoPath =
        !empty($logoName) && File::exists(public_path("storage/logo/$logoName"))
        ...
        <div class="row-top">
            ...
            <div class="search-box hidden-xs hidden-sm hidden-md">
                ...
                <div class="search-container">
                    <input type="text" name="pe" id="product-search"
                        value="{{ session('filters.searchTerms') ?? null }}" placeholder="O que você procura?"
                        autocomplete="off" oninput="handleSearch(this.value)" onfocus="showSuggestions()"
            ...
            <div class="sign-in hidden-xs hidden-sm hidden-md">
                @if (!auth()->guard('buyer')->user() && !auth()->guard(name: 'seller')->user())
                ...
                @elseif (auth()->guard('seller')->user())
                    <a href="{{ route('seller.clients') }}" class="logged-in" title="Acessar painel do cliente">
                        ...
                        <span>
                            ...
                            {{ session('buyer.clients.selected') instanceof \App\Models\Client
                                ? \Illuminate\Support\Str::limit(
                                    session('buyer.clients.selected')?->company_name ?? session('buyer.clients.selected')?->name,
                                    10,
                                )
                    ...
                    <div id="cart-box" class="loading" data-route="{{ route('cart.html') }}">
                        <div class="left-side">
                            ...
                            <a href="{{ route('cart.index') }}" class="right-side">
                                <svg xmlns="http://www.w3.org/2000/svg" width="19.006" height="18.21"
                                    viewBox="0 0 19.006 18.21">
                                    <g transform="translate(-0.75 -0.75)">
                                        <path fill="none" stroke-width="1.7px" stroke-linejoin="round"
                                            stroke-linecap="round" stroke="currentColor"
                                            d="M13.591,30.8a.8.8,0,1,1-.8-.8A.8.8,0,0,1,13.591,30.8Z"
                                            transform="translate(-4.93 -13.381)"></path>
                                        <path fill="none" stroke-width="1.7px" stroke-linejoin="round"
                                            stroke-linecap="round" stroke="currentColor"
                                            d="M30.091,30.8a.8.8,0,1,1-.8-.8A.8.8,0,0,1,30.091,30.8Z"
                                            transform="translate(-12.677 -13.381)"></path>
                                        <path fill="none" stroke-width="1.7px" stroke-linejoin="round"
                                            stroke-linecap="round" stroke="currentColor"
                ...
                <div class="container">
                    <ul>
                        @foreach ($navigation->links->sortBy('order') as $link)
                            <li>
                                <a href="{{ $link->isExternal() ? $link->url : url($link?->url) }}"
