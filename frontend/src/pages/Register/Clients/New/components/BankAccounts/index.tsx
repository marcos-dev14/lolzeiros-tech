...
export function BankAccounts({ bankAccounts }: Props) {
  ...
  const handleAddBankAccount = useCallback(async () => {
    try {
      ...
      } = await api.post(`/clients/${id}/bank_accounts`, bankAccountData);
      ...
      updateClient({ bank_accounts: bankAccounts });
    } catch (e) {
      console.log('e', e);
      // @ts-ignore
  ...
  const handleEditBankAccount = useCallback(async (order) => {
    try {
      ...
      const {
        data: { data }
      } = await api.post(`/clients/${id}/bank_accounts/${order.id}?_method=PUT`, order);
  ...
  const handleDeleteBankAccount = useCallback(async () => {
    try {      
      ...
      await api.delete(`/clients/${id}/bank_accounts/${deletingBankAccount}`);
  ...
  }, [client, updateClient, currentData, deletingBankAccount]);
