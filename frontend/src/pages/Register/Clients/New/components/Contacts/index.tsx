...
import { capitalizeContent, isMailValid, phoneIsValid, landlineIsValid } from '@/src/utils/validation';
...
export function Contacts({ contacts }: Props) {
  ...
  const handleAddContact = useCallback(async () => {
    try {
      ...
      } = await api.post(`/clients/${id}/contacts`, contactData);
  ...
  const handleEditContact = useCallback(async (order) => {
    try {
      ...
      const {
        data: { data }
      } = await api.post(`/clients/${id}/contacts/${order.id}?_method=PUT`, order);
  ...
  const handleDeleteContact = useCallback(async () => {
    try {      
      ...
      await api.delete(`/clients/${id}/contacts/${deletingContact}`);
  ...
  }, [client, updateClient, currentData, deletingContact]);
      ...
      {currentData.map((c) =>
        ...
        >
          <TableContent>
            ...
            <div>
              <TableActionButton
                disabled={deletingContact === c.id || c.name === 'Api Receita'}
