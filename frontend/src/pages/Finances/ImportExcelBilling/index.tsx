...
export function ImportExcelBilling() {
  ...
  const [modelFileDownload, setModelFileDownload] = useState<File | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data } = await api.post(
        `/invoicing/import-excel/${supplierSelected}`,
        formData,
        {
          headers: {
  ...
  //     const { data } = await api.get('/invoicing/download-excel', {
  //       responseType: 'blob', // para lidar com o retorno do arquivo
  //     });
  ...
  //     } else {
  //       setError(
  //         'Erro ao baixar o arquivo modelo. Tente novamente mais tarde.'
  //       );
  //     }
  //   } finally {
  //     setLoading(false);
  ...
  return (
    <>
      <Header route={['Importação de Excel - Faturamentos']} />
      <MenuAndTableContainer>
        <Menu />
        <Container>
          {supplierSelected ? (
      ...
      >
        <h2 style={{ color: '#0C5DA7', fontSize: '24px' }}>
          Importação de Excel - Faturamentos
        </h2>
        <form onSubmit={handleSubmit} style={{ marginBottom: '0.5rem' }}>
          <InputFileContainer>
            ...
            <DownloadFileButton route="/invoicing/download-excel" />
            <Button
              style={{
          ...
          >
            ...
            <tbody style={{ backgroundColor: '#F5F5F5' }}>
              {orders?.map((order, index) => (
                <tr key={index}>
                  <td style={{ fontSize: '1rem', lineHeight: '125%', whiteSpace: 'nowrap' }}>
                    {order.order_code}
                  </td>
                  <td
                    style={{
        ...
        <div style={{ alignItems: 'center', display: 'flex', width: '100%' }}>
          <Button
            style={{
              ...
              margin: 'auto',
              marginTop: '1rem',
              cursor: selectedOrders ? 'pointer' : 'not-allowed', // Altera o cursor para indicar se o botão está habilitado
            }}
            onClick={handleDownloadLog} // Remova os parênteses para passar a função corretamente
            disabled={!selectedOrders} // Desativa o botão se selectedOrders não estiver definido
          >
            Download do log
          </Button>
        </div>
